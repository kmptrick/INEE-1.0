          ...
          {!forgotOpen ? (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5" style={{ color: '#6B4C35' }}>Email</label>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="block text-xs font-semibold uppercase tracking-wider" style={{ color: '#6B4C35' }}>Mot de passe</label>
                    <button type="button" onClick={openForgot}
                      className="text-xs transition-colors"
                      style={{ color: '#D9924E' }}
                      onMouseEnter={e => (e.currentTarget.style.color = '#A06828')}
                      onMouseLeave={e => (e.currentTarget.style.color = '#D9924E')}>
                      Mot de passe oublié ?
                    </button>
                  </div>
                ...
                {locked && (
                  <div className="rounded-lg px-4 py-3 space-y-2" style={{ background: '#FEF2F2', border: '1px solid #FECACA' }}>
                    ...
                    <button type="button" onClick={openForgot} className="text-xs font-semibold underline cursor-pointer" style={{ color: '#D9924E' }}>
                ...
                <button type="submit" disabled={loading}
                  ...
                  onMouseEnter={e => { if (!loading) (e.currentTarget as HTMLElement).style.background = '#C07E3A'; }}
                  onMouseLeave={e => { if (!loading) (e.currentTarget as HTMLElement).style.background = '#D9924E'; }}
                >
            ...
            <>
              <button onClick={() => setForgotOpen(false)} className="flex items-center gap-1.5 text-xs mb-6 transition-colors" style={{ color: '#8A7060' }}
                onMouseEnter={e => (e.currentTarget.style.color = '#D9924E')}
              ...
              {forgotDone ? (
                <div className="rounded-xl px-5 py-6 text-center space-y-3" style={{ background: '#F0FDF4', border: '1px solid #BBF7D0' }}>
                  ...
                  <p className="text-xs" style={{ color: '#6B4C35' }}>Si cet email existe dans notre système, vous recevrez un lien dans quelques minutes.</p>
                ...
                <form onSubmit={handleForgot} className="space-y-5">
                  ...
                  <button type="submit" disabled={forgotLoading}
                    className="w-full font-semibold py-3 px-4 rounded-lg text-sm transition-all"
                    style={{ background: forgotLoading ? '#A06828' : '#D9924E', color: '#FFFFFF', letterSpacing: '0.03em', boxShadow: '0 2px 8px rgba(200,128,58,0.35)' }}>
