...
export class InvoicingController {
  ...
  @Get('quotes')
  findAllQuotes(@Query('status') status?: string) {
    return this.service.findAllQuotes(status);
  }

  @Get('quotes/:id')
  findOneQuote(@Param('id') id: string) {
    return this.service.findOneQuote(id);
  }

  @Post('quotes')
  createQuote(@Body() dto: CreateQuoteDto, @CurrentUser() user: { sub: string }) {
    return this.service.createQuote(dto, user.sub);
  }
  ...
  removeQuote(@Param('id') id: string) {
    return this.service.removeQuote(id);
  }
  ...
  findAllInvoices(@Query('status') status?: string) {
    return this.service.findAllInvoices(status);
  }
  ...
  @Post('invoices')
  createInvoice(@Body() dto: CreateInvoiceDto, @CurrentUser() user: { sub: string }) {
    return this.service.createInvoice(dto, user.sub);
  }

  @Post('invoices/from-quote')
  createFromQuote(@Body() dto: CreateInvoiceFromQuoteDto, @CurrentUser() user: { sub: string }) {
    return this.service.createInvoiceFromQuote(dto, user.sub);
  }
